...
try {
  ...
  const { rows } = await client.query(
    `insert into casa_studies (slug, title, status, position) values ($1, '', 'draft', 0) returning id, slug, title`,
    [slug]
  );
  console.log("Insertado correctamente:", rows[0]);
  await client.query(`delete from casa_studies where id = $1`, [rows[0].id]);
  console.log("Limpiado (era solo una prueba).");
} finally {
  await client.end();
}
