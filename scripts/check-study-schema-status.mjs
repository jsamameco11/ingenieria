...
try {
  const { rows } = await client.query(`
    ...
    where table_schema = 'public' and table_name in
      ('casa_studies','casa_study_sections','casa_study_lessons','casa_study_contents','casa_study_content_verses')
    order by table_name
  `);
  console.log("Tablas existentes:");
  console.table(rows);

  const { rows: studyCols } = await client.query(`
    select column_name from information_schema.columns
    where table_schema='public' and table_name='casa_studies'
    order by ordinal_position
  `);
  console.log("Columnas de casa_studies:");
  console.table(studyCols);

  const { rows: sectionCols } = await client.query(`
    select column_name from information_schema.columns
    where table_schema='public' and table_name='casa_study_sections'
    order by ordinal_position
  `);
  console.log("Columnas de casa_study_sections:");
