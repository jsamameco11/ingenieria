...
try {
  const { rows: translations } = await client.query(
    `select code, name, is_active, license_type from casa_bible_translations order by position`
  );
  console.log("Traducciones disponibles:");
  console.table(translations);

  const refs = [
    { book: "genesis", chapter: 12, verse: 1 },
    { book: "genesis", chapter: 15, verse: 6 },
    { book: "romanos", chapter: 4, verse: 20 },
  ];

  for (const t of translations.filter((t) => t.is_active)) {
    console.log(`\n--- ${t.code} ---`);
    for (const r of refs) {
      const { rows } = await client.query(
        `select v.text
          ...
          where b.slug = $1 and c.chapter_number = $2 and v.verse_number = $3 and tr.code = $4`,
        [r.book, r.chapter, r.verse, t.code]
      );
      console.log(`${r.book} ${r.chapter}:${r.verse} ->`, rows[0]?.text ?? "(no encontrado)");
    }
  }
} finally {
  await client.end();
