...
try {
  const { rows } = await client.query(
    `select id, email,
            ...
            array(select provider from auth.identities where user_id = auth.users.id) as providers,
            created_at
       from auth.users
      where email = $1`,
    [EMAIL]
  );
  console.log(`auth.users para ${EMAIL}:`);
  console.table(rows);

  if (rows.length > 0) {
    const { rows: profileRows } = await client.query(
      `select id, role from casa_profiles where id = $1`,
      [rows[0].id]
    );
    console.log("casa_profiles:");
