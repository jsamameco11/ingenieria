...
try {
  const { rows } = await client.query(
    `select v.verse_number, v.text
      ...
      where b.slug = 'genesis' and c.chapter_number = 12 and v.verse_number between 1 and 3 and tr.code = 'RVR1909'
      order by v.verse_number`
  );
  if (rows.length !== 3) throw new Error(`Esperaba 3 versículos, encontré ${rows.length}.`);
  const text = rows.map((r) => r.text).join(" ");
  console.log("Génesis 12:1-3 (RVR1909):", text);

  const { rowCount } = await client.query(
    `update casa_study_content_verses v
        set text = $1, translation_code = 'RVR1909'
       from casa_study_contents c, casa_study_sections s, casa_studies st
      where v.content_id = c.id and c.section_id = s.id and s.study_id = st.id
        and st.slug = $2 and v.book_slug = 'genesis' and v.chapter_start = 12
        and v.verse_start = 1 and v.verse_end = 3`,
    [text, SLUG]
  );
  console.log(`Texto agregado al bloque de versículo (${rowCount} fila).`);
} finally {
  await client.end();
}
