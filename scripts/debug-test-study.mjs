...
try {
  const { rows } = await client.query(`select id, slug, status from casa_studies where slug = $1`, [SLUG]);
  console.log("Estudio (vista directa, sin RLS):");
  console.table(rows);

  // Simula exactamente lo que ve el rol anon bajo RLS.
  await client.query(`set role anon`);
  const { rows: anonRows } = await client.query(`select id, slug, status from casa_studies where slug = $1`, [SLUG]);
  console.log("Lo que ve el rol anon (con RLS aplicada):");
  console.table(anonRows);
