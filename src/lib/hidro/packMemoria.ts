...
export function buildBocatoma(meta: MetaPack, r: ReturnType<typeof calcularBocatoma>): MemoriaDoc {
  ...
  return {
    ...
    blocks: [
      ...
      paso("5.5", "Barraje y carga de diseño", "P = ho + h + hs", `Lbarraje = ${fmt(r.Lbarraje, 1)} m`, `P = ${fmt(r.P, 2)} m · Cc = ${fmt(r.Cc, 2)} msnm · He = ${fmt(r.He, 2)} m · H total sobre cimentación = ${fmt(r.Hbarraje, 2)} m`),
      { type: "h2", text: "6. Socavación general (Lischtvan–Lebediev)" },
      { type: "p", text: "Método estándar del MTC (Manual de Hidrología, Hidráulica y Drenaje) para cauces no cohesivos: iguala la velocidad real con la velocidad erosiva del material del lecho bajo el caudal de avenida." },
      { type: "eq", text: "Hs = [ qd / (0.68 β Dm^{0.28}) ]^{1/(1+x)}     ·     qd = Qmáx / (Be·μ)", num: "4" },
      paso(
        "6.1",
        "Profundidad de socavación",
        "Hs = [qd/(0,68 β Dm^0,28)]^(1/(1+x))",
        `qd=${fmt(r.qd, 2)} m³/s/m · β(Tr=${fmt(r.Tr, 0)} años)=${fmt(r.beta, 2)} · x(Dm=${fmt(r.d50, 1)} mm)=${fmt(r.x, 3)}`,
        `Hs = ${fmt(r.Hs, 2)} m (medida desde la superficie) · socavación neta bajo el lecho = ${fmt(r.socNeta, 2)} m`,
        "Hs no depende del tirante original: el método asume que la erosión se detiene cuando la velocidad real iguala a la erosiva, condición que solo depende del caudal unitario y del material — el tirante inicial solo se usa para restar y obtener la profundidad neta bajo el lecho existente.",
      ),
      { type: "check", ok: true, text: `La zapata y el dentellón aguas abajo deben desplantarse al menos ${fmt(r.profZapata, 2)} m bajo el lecho actual (socavación neta + 0,50 m de resguardo).` },
      { type: "h2", text: "7. Estabilidad del barraje (sección de gravedad, método de Lane)" },
      { type: "p", text: "El barraje se verifica como una pequeña presa de gravedad: talón vertical aguas arriba, cara de descarga con talud, subpresión por infiltración bajo la base (línea de infiltración ponderada de Lane) y verificación de deslizamiento, volteo, excentricidad y presión de contacto." },
      { type: "eq", text: "Lw = Lv + ⅓ Lh (Lane)     ·     Lw ≥ Cw·H     ·     U = ½(p_talón+p_pie)·B", num: "5" },
      paso(
        "7.1",
        "Línea de infiltración ponderada y subpresión",
        "Lw=cutoffUp+cutoffDown+⅓B+⅓Ldescarga ≥ Cw·H",
        `Cw(${LANE_CW.find((s) => s.id === r.tipoSuelo)?.label ?? r.tipoSuelo})=${fmt(r.CwLane, 2)} · H=${fmt(r.Hbarraje, 2)} m`,
        `Lw=${fmt(r.LwCreep, 2)} m ${r.okPercolacion ? "≥" : "<"} Lw,req=${fmt(r.LwReq, 2)} m · Subpresión U=${fmt(r.Usub, 1)} t/m`,
        "Cw pondera el material de cimentación: entre más fino, más larga debe ser la trayectoria de infiltración para no arrastrar partículas (tubificación/piping). Los tramos verticales (dentellones) cuentan al 100%; los tramos casi horizontales (base, taludes tendidos) solo al ⅓, porque el gradiente de infiltración es menos efectivo para erosionar en esa dirección.",
      ),
      paso(
        "7.2",
        "Deslizamiento y volteo",
        "FSd=(W−U)tanφ/Ph ≥ 1,5     ·     FSv=Mr/Ma ≥ 1,5",
        `W=${fmt(r.Wbarraje, 1)} t/m · U=${fmt(r.Usub, 1)} t/m · Ph=${fmt(r.PhidroH, 1)} t/m · φcim=${fmt(r.phiCimentacion, 1)}°`,
        `FSd=${fmt(r.FSdeslBarraje, 2)} · FSv=${fmt(r.FSvoltBarraje, 2)}`,
        undefined,
        ["W es el peso propio de la sección trapezoidal del barraje; U (subpresión) reduce el peso efectivo disponible para generar fricción resistente en la base."],
      ),
      paso(
        "7.3",
        "Excentricidad y presión de contacto",
        "e=B/2−(Mr−Ma)/(W−U) ≤ B/6     ·     qmáx=(W−U)/B·(1+6e/B)",
        `B=${fmt(r.Btot, 2)} m`,
        `e=${fmt(r.eBarraje, 3)} m (${Math.abs(r.eBarraje) <= r.Btot / 6 ? "dentro" : "fuera"} del núcleo) · qmáx=${fmt(r.qmaxBarraje, 1)} t/m² (${fmt(r.qmaxBarraje / 10, 2)} kg/cm²)`,
      ),
      { type: "check", ok: r.okBarraje, text: r.okBarraje ? "El barraje es estable: percolación, deslizamiento, volteo, excentricidad y presión de contacto cumplen." : "Revisar: aumentar la base B, profundizar los dentellones o mejorar el suelo de cimentación." },
      { type: "h2", text: "8. Perfil de remanso (método de paso directo)" },
      { type: "p", text: "Se integra la ecuación de flujo gradualmente variado por pasos, desde el tirante en el barraje hasta recuperar el tirante normal del río, en vez de estimar la longitud como Ymáx/S (que sobrestima o subestima según qué tan lejos esté el tirante normal del crítico)." },
      paso(
        "8.1",
        "Longitud de remanso",
        "Δx=(E2−E1)/(S0−S̄f), sumado tramo a tramo hasta y≈1,01·yn",
        `yn(Manning, n=0,035 est.)=${fmt(r.ynRemanso, 2)} m · tirante en el barraje=${fmt(r.Hbarraje, 2)} m`,
        `L remanso ≈ ${fmt(r.Lremanso, 1)} m`,
        "Esta es la distancia aguas arriba del barraje donde el nivel del río deja de estar influenciado por la represa — relevante para la servidumbre de inundación y para saber hasta dónde revisar la erosión de las márgenes.",
      ),
      { type: "h2", text: "9. Desripiador (purga de fondo)" },
      paso(
        "9.1",
        "Compuerta de purga",
        "Q=Cd·A·√(2gP)     ·     V=Q/A ≥ 1,0 m/s (arrastre de sedimento grueso)",
        `b=${fmt(r.bDesrip, 2)} m · h=${fmt(r.hDesrip, 2)} m · Cd=${fmt(r.CdDesrip, 2)} · P=${fmt(r.P, 2)} m`,
        `Q desripiador=${fmt(r.QDesrip, 2)} m³/s · V=${fmt(r.VDesrip, 2)} m/s`,
        "El desripiador purga el material grueso (gravas, cantos) que decanta frente a la ventana de captación antes de que llegue al desarenador (HID-06), que solo está diseñado para arenas y limos finos.",
      ),
      { type: "check", ok: r.okDesrip, text: r.okDesrip ? "La compuerta de purga alcanza velocidad de arrastre suficiente." : "Aumentar la carga (P) o el área de la compuerta: V < 1,0 m/s no remueve gravas." },
      { type: "h2", text: "10. Conclusión" },
      { type: "p", text: `Ventana de ${fmt(r.bTotal, 2)} m con ${r.nBarr} barrotes, altura ${fmt(r.hOrif, 2)} m. Cresta en ${fmt(r.Cc, 2)} msnm. ht muro ${fmt(r.htMuro, 2)} m. Barraje B=${fmt(r.Btot, 2)} m: FSd=${fmt(r.FSdeslBarraje, 2)}, FSv=${fmt(r.FSvoltBarraje, 2)}. Socavación neta ${fmt(r.socNeta, 2)} m → desplante de zapata ≥ ${fmt(r.profZapata, 2)} m. Remanso ≈ ${fmt(r.Lremanso, 1)} m.` },
    ],
  };
}
...
export function buildRapida(meta: MetaPack, r: ReturnType<typeof calcularRapida>): MemoriaDoc {
  ...
  return {
    ...
    blocks: [
      ...
      { type: "h2", text: "7. Cuenco disipador USBR — dimensionamiento de apéndices" },
      { type: "p", text: "El tipo de cuenco por Fr₁ ya se adopta en el paso 5.4; aquí se predimensionan sus apéndices (dados de entrada, contradados y solera terminal) con las reglas proporcionales usuales de USBR (Peterka, 1978). El ajuste fino de posición y forma exactos debe verificarse contra las cartas de diseño del EM-25 antes de construir." },
      { type: "eq", text: "Dados de entrada: h = w = s ≈ y1     ·     Contradados (cuenco III): 0,75 y1 ≤ h3 ≤ y2/6, aptos si Fr1∈[4,5;17], q≤18,6 m²/s y V1≤15 m/s     ·     Solera terminal: hs ≈ 0,2 y2", num: "2" },
      paso(
        "7.1",
        "Dados de entrada (chute blocks)",
        "h = w = s ≈ y1",
        `y1 = ${fmt(r.y1, 3)} m · b = ${fmt(r.b, 2)} m`,
        `${r.nBloquesEntrada} dados de ${fmt(r.hBloqueEntrada, 2)} × ${fmt(r.wBloqueEntrada, 2)} m, separados ${fmt(r.sBloqueEntrada, 2)} m`,
        "Los dados de entrada rompen el chorro y adelantan el resalto, acortando el cuenco necesario respecto al de un resalto libre en solera lisa.",
      ),
      paso(
        "7.2",
        "Contradados (cuenco USBR III)",
        "Aptos si Fr1∈[4,5;17], q≤18,6 m²/s, V1≤15 m/s",
        `Fr1 = ${fmt(r.Fr1, 2)} · q = ${fmt(r.q, 2)} m²/s · V1 = ${fmt(r.V1, 2)} m/s`,
        r.aptoContradado
          ? `Aplicables: altura entre ${fmt(r.hContradadoMin, 2)} y ${fmt(r.hContradadoMax, 2)} m → cuenco corto, L ≈ ${fmt(r.LcuencoUSBR, 2)} m`
          : `No aplicables (cuenco ${r.usbrTipo} o q/V1 fuera de rango) → usar cuenco Tipo II sin contradados, L ≈ ${fmt(r.Lres, 2)} m (cavitación en los contradados a alta velocidad)`,
        "Los contradados (baffle piers) acortan el cuenco absorbiendo cantidad de movimiento por impacto directo, pero a velocidades de entrada altas la caída de presión en su cara aguas abajo puede cavitar el concreto — por eso USBR los excluye fuera del rango V1≤15 m/s.",
      ),
      paso(
        "7.3",
        "Solera terminal y longitud del cuenco",
        "hs ≈ 0,2 y2     ·     L cuenco (con dados y contradados) < Lr (resalto libre)",
        `y2 = ${fmt(r.y2, 3)} m`,
        `hs ≈ ${fmt(r.hSolera, 2)} m · L cuenco adoptado ≈ ${fmt(r.LcuencoUSBR, 2)} m (resalto libre sin apéndices requeriría ${fmt(r.Lres, 2)} m)`,
      ),
      { type: "h2", text: "8. Protección de salida — enrocado (escollera)" },
      { type: "p", text: "Aguas abajo de la solera terminal, la corriente aún lleva turbulencia residual: se protege el cauce con un colchón de enrocado dimensionado por la fórmula de Izbash (coeficiente de alta turbulencia, apropiado a la salida de un resalto hidráulico)." },
      { type: "eq", text: "D50 = V² / [2 g (Ss−1) K²]     (Izbash, K=0,86 alta turbulencia)     ·     L delantal ≈ 5 y2", num: "3" },
      paso(
        "8.1",
        "Tamaño de roca y longitud del delantal",
        "D50 = V2² / [2g(Ss−1)K²]",
        `V2 = Q/(b·y2) = ${fmt(r.V2apron, 2)} m/s · Ss = ${fmt(r.GsRip, 2)}`,
        `D50 ≈ ${fmt(r.D50rip * 100, 1)} cm · L delantal ≈ ${fmt(r.Lapron, 2)} m`,
        "V2 (velocidad de salida del cuenco) gobierna el tamaño de roca porque, aunque el resalto disipa la mayor parte de la energía, la turbulencia residual a la salida sigue siendo capaz de mover partículas del lecho natural si no se protege.",
      ),
      { type: "h2", text: "9. Conclusión" },
      { type: "p", text: `Rápida b = ${fmt(r.b, 2)} m, desnivel ${fmt(r.Hdesnivel, 2)} m. Pozo y₂ = ${fmt(r.y2, 2)} m. Cuenco ${r.tipoUSBR}${r.aptoContradado ? " con contradados" : ""}, L ≈ ${fmt(r.LcuencoUSBR, 2)} m. Enrocado de salida D50 ≈ ${fmt(r.D50rip * 100, 0)} cm en ${fmt(r.Lapron, 1)} m.` },
    ],
  };
}
