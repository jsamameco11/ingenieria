...
function bordes(sec: Seccion): { inicio: number; fin: number; partes: ParteVia[] } {
  const partesSrc = partesDeSeccion(sec);
  const partes: ParteVia[] = [];
  let cursor = 0;
  for (const parte of partesSrc) {
    partes.push({ tipo: parte.tipo, a: cursor, b: cursor + parte.ancho });
    cursor += parte.ancho;
  }
...
function resolver(id: string, baseTipo: TipoVia, base: Seccion, ajustes: AjusteVia[]) {
  ...
  const seccion = aj?.seccion ?? base;
  const b = bordes(seccion);
  return {
    tipo,
    seccion,
    ancho: anchoSeccion(seccion),
  ...
  for (let i = 0; i < repH.sizes.length; i++) {
    ...
    if (i < nH) {
      ...
      hCalles.push({
        ...
        pos: y,
        span,
        partes: bordes({ ...r.seccion }).partes.map((p) => ({
          ...p,
...
export function esquinasDe(hCalles: CallePuesta[], vCalles: CallePuesta[]): Esquina[] {
  ...
  for (const hc of hCalles) {
    for (const vc of vCalles) {
      ...
      const seeds = [
        { x: vc.pos, y: hc.pos, sx: 1 as const, sy: 1 as const, wV: vc.veredaInicio, wH: hc.veredaInicio },
        { x: vc.pos + vc.span, y: hc.pos, sx: -1 as const, sy: 1 as const, wV: vc.veredaFin, wH: hc.veredaInicio },
        { x: vc.pos, y: hc.pos + hc.span, sx: 1 as const, sy: -1 as const, wV: vc.veredaInicio, wH: hc.veredaFin },
        { x: vc.pos + vc.span, y: hc.pos + hc.span, sx: -1 as const, sy: -1 as const, wV: vc.veredaFin, wH: hc.veredaFin },
      ];
      for (const s of seeds) {
        const e = hacerEsquina({ x: s.x, y: s.y }, s.sx, s.sy, s.wH, s.wV, R);
        if (e) out.push(e);
      }
    }
  }
